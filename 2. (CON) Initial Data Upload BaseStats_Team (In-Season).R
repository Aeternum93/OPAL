...
# 2.3 Load team box from PBP game IDs using espn_nba_team_box() (replaces load_nba_team_box)
pbp_path <- paste0(
  "C:/Users/Austin/OneDrive/Desktop/1/Data Analytics/NBA Data/0. Datahub (Temp)/1. hoopR/7. MC PBP Data/nbapbp_",
  season_token, ".csv"
)
pbp_games <- data.table::fread(pbp_path, select = "game_id") %>% dplyr::distinct()
espn_game_ids <- unique(as.character(pbp_games$game_id))



# Filter to only pull games for dates NOT already in BaseStats_Team
if (nrow(BaseStats_Team) > 0) {
  existing_dates <- unique(BaseStats_Team$game_date)
  
  # Get game_date for each game_id from PBP file
  ...
  # Only keep game IDs for dates we don't have yet
  ...
  message("BaseStats_Team has through ", max(existing_dates), 
          ". Only pulling ", length(missing_game_ids), " new game(s).")
  espn_game_ids <- missing_game_ids
}


nba_team_box <- data.frame()
for (gid in espn_game_ids) {
  message("Pulling team box for game_id: ", gid, " (", which(espn_game_ids == gid), " of ", length(espn_game_ids), ")")
  
  tb <- tryCatch({
    espn_nba_team_box(game_id = gid)
  }, error = function(e) {
    message("  Error for game_id: ", gid, " -> ", e$message)
    return(NULL)
  })
  if (!is.null(tb) && nrow(tb) > 0) {
    nba_team_box <- dplyr::bind_rows(nba_team_box, tb)
  }
  Sys.sleep(0.50)
}
