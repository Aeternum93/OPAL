...
# ESPN headshot URL builder from athlete ESPN IDs
espn_headshot_url <- function(espn_id) {
  if(is.na(espn_id) || espn_id == "" || espn_id == 0) return("")
  paste0("https://a.espncdn.com/i/headshots/nba/players/full/", espn_id, ".png")
}
...
table.sc-lineup-table { width:100%; border-collapse:collapse; font-family:'Share Tech Mono',monospace; font-size:12px; }
table.sc-lineup-table thead th { padding:6px 8px; text-align:left; font-size:10px; letter-spacing:.08em; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:400; text-transform:uppercase; }
table.sc-lineup-table tbody td { padding:6px 8px; color:var(--text-secondary); border-bottom:1px solid rgba(255,255,255,.04); }
table.sc-lineup-table tbody tr:hover { background:var(--bg-card-hover); }

/* ── MINI CALENDAR (compact version for Shot Chart / Game Calendar) ── */
...
.gc-score-center { display:flex; align-items:center; gap:12px; }
...
.gc-status { font-size:11px; font-family:'Share Tech Mono',monospace; color:var(--text-muted); letter-spacing:.08em; text-align:center; margin-top:4px; }
...
.gc-play-logo { width:28px; height:28px; object-fit:contain; flex-shrink:0; margin-top:2px; }
...
.gc-play-assist { font-size:11px; color:var(--text-muted); font-family:'Share Tech Mono',monospace; margin-top:3px; }
...
server <- function(input, output, session) {
  ...
  # ── Tool-level tab state (for Player Stats / Team Stats top tabs) ──
  ps_tool_tab   <- reactiveVal("search")      # "search", "shotchart", "gamecal", "gamecast"
  ts_tool_tab   <- reactiveVal("search")      # "search", "shotchart", "gamecal", "gamecast"
  
  # ── Shot Chart state ──
  sc_game_id    <- reactiveVal(NULL)           # selected game ESPN ID
  sc_qtr_filter <- reactiveVal("ALL")          # quarter filter
  sc_type_filter <- reactiveVal("ALL")         # play type filter
  sc_player_filter <- reactiveVal("ALL")       # player filter
  ...
  observeEvent(input$sc_game_select, { sc_game_id(input$sc_game_select) })
  ...
  # GameCast observers
  observeEvent(input$gc_game_select, { gc_game_id(input$gc_game_select) })
  ...
  observeEvent(input$gcal_game_click, {
    # Route to GameCast for that game
    gc_game_id(input$gcal_game_click)
    # If we're inside Player Stats, switch to GameCast tab
    if(!is.null(sub_active()) && sub_active() == "player_stats") {
      ps_tool_tab("gamecast")
      session$sendCustomMessage("setToolTab", list(group="ps", tab="gamecast"))
    } else if(!is.null(sub_active()) && sub_active() == "team_stats") {
      ts_tool_tab("gamecast")
      session$sendCustomMessage("setToolTab", list(group="ts", tab="gamecast"))
    }
  })
    ...
    if (!is.null(sub_active()) && sub_active() == "player_stats") {
      ...
      return(tags$div(class="fade-in",
                      ...
                      if(ps_tab == "search") {
                        ...
                        uiOutput("sc_ui_player")
                      } else if(ps_tab == "gamecal") {
                        uiOutput("gcal_ui_player")
  ...
  # ════════════════════════════════════════════════════════════════════════════
  # SHOT DETAIL CHART — shared between Player Stats & Team Stats
  # ════════════════════════════════════════════════════════════════════════════
  
  # Helper: build mini calendar HTML
  render_mini_cal <- function(y, m, game_dates, selected_date, input_prefix) {
    ...
    days_html <- paste(sapply(0:41, function(i) {
      ...
      is_today <- cd == today
      cls <- paste("mini-cal-day",
                   if(is_cur) "cur-month" else "",
                   if(has_game) "has-game" else "",
      ...
      sprintf("<div class='%s'%s>%s</div>", trimws(cls), onclick, as.integer(format(cd, "%%d")))
    }), collapse="")
    
    sprintf("<div class='mini-cal-wrap'>%s<div class='mini-cal-grid'>%s%s</div></div>", nav_html, dow_html, days_html)
  }
  
  # ── Shot Chart UI (Player Stats version — shows all games, no team filter) ──
  output$sc_ui_player <- renderUI({
    # Get all unique game dates from PBP
    all_dates <- sort(unique(pbp_data$game_date))
    sel_date_str <- sc_game_id()  # we store date string here from mini-cal click
    sel_date <- if(!is.null(sel_date_str)) as.Date(sel_date_str) else NULL
    
    tagList(
      tags$div(class="sc-layout",
               tags$div(class="sc-top-row",
                        # Left: mini cal + filters + court
                        tags$div(class="sc-court-col",
                                 HTML(render_mini_cal(sc_cal_year(), sc_cal_month(), all_dates, sel_date, "sc")),
                                 if(!is.null(sel_date)) uiOutput("sc_game_picker_ui") else NULL,
                                 if(!is.null(sc_game_id()) && !is.null(sel_date)) tagList(
                                   uiOutput("sc_filters_ui"),
                                   uiOutput("sc_court_ui")
                                 ) else tags$div(class="pr-placeholder", style="margin-top:40px;", tags$p("Select a date with games to view shot chart"))
                        ),
                        # Right: team stats
                        if(!is.null(sc_game_id())) tags$div(class="sc-sidebar-col", uiOutput("sc_team_stats_ui"), uiOutput("sc_game_leaders_ui"))
               ),
               # Bottom: on-court lineup table
               if(!is.null(sc_game_id())) uiOutput("sc_oncourt_ui")
      )
    )
  })
  
  # ── Shot Chart UI (Team Stats version — identical, reuses same outputs) ──
  output$sc_ui_team <- renderUI({
    all_dates <- sort(unique(pbp_data$game_date))
  ...
  # ── Game picker (when date is selected, show games on that date) ──
  output$sc_game_picker_ui <- renderUI({
    sel_date <- as.Date(sc_game_id())
    ...
    choices <- setNames(as.character(day_games$game_id),
                        paste(day_games$away_team_abbrev, "@", day_games$home_team_abbrev))
  ...
  # Reactive: filtered PBP for current shot chart
  sc_pbp <- reactive({
    gid <- sc_picked_game()
    if(is.null(gid) || gid == "") {
      # Try first game on selected date
  ...
  output$sc_filters_ui <- renderUI({
    ...
    # Player options — get unique players from both teams
    away_abv <- df$away_team_abbrev[1]; home_abv <- df$home_team_abbrev[1]
    players <- df %>% filter(PLAYER_NAME != "" & !is.na(PLAYER_NAME)) %>%
      distinct(PLAYER_NAME, team_id) %>% arrange(PLAYER_NAME)
    ...
    tags$div(class="sc-filters",
             # Quarter
             tags$select(class="sc-filter-select",
                         onchange="Shiny.setInputValue('sc_qtr_click',this.value,{priority:'event'})",
                         tags$option(value="ALL", "All Quarters"),
                         lapply(seq_along(qtrs), function(i) tags$option(value=qtrs[i], qtr_labels[i]))
             ),
             # Play Type
             tags$select(class="sc-filter-select",
                         onchange="Shiny.setInputValue('sc_type_sel',this.value,{priority:'event'})",
                         tags$option(value="ALL","All Play Types"),
                         tags$option(value="2PT FG Made","2PT FG Made"),
                         tags$option(value="2PT FG Missed","2PT FG Missed"),
                         tags$option(value="3PT Made","3PT Made"),
                         tags$option(value="3PT Missed","3PT Missed")
             ),
             # Player
             tags$select(class="sc-filter-select",
                         onchange="Shiny.setInputValue('sc_player_sel',this.value,{priority:'event'})",
                         tags$option(value="ALL","All Players"),
                         tags$optgroup(label=away_abv,
                                       lapply(seq_len(nrow(away_players)), function(i) tags$option(value=away_players$PLAYER_NAME[i], away_players$PLAYER_NAME[i]))
                         ),
                         tags$optgroup(label=home_abv,
                                       lapply(seq_len(nrow(home_players)), function(i) tags$option(value=home_players$PLAYER_NAME[i], home_players$PLAYER_NAME[i]))
                         )
             )
    )
  })
  ...
  output$sc_court_ui <- renderUI({
    ...
    qf <- sc_qtr_filter()
    if(qf != "ALL") shots <- shots %>% filter(qtr == as.integer(qf))
    tf <- sc_type_filter()
    if(tf != "ALL") shots <- shots %>% filter(shot_category == tf)
    pf <- sc_player_filter()
    if(pf != "ALL") shots <- shots %>% filter(PLAYER_NAME == pf)
    
    # NBA court coordinates: LOC_X ranges ~-250 to 250, LOC_Y ranges ~-50 to 420
    # We'll draw a half court 500 wide x 470 tall, offset so (0,0) is at basket
    # SVG viewBox: -250 -52 500 472
    
    # Team colors (simplified — just use dark/red for away, dark/blue for home)
    away_color <- "#e53935"  # red
    home_color <- "#1565c0"  # blue
    
    home_logo_url <- paste0("https://a.espncdn.com/i/teamlogos/nba/500/scoreboard/", tolower(home_abv), ".png")
    ...
    # Shot dots
    dots_svg <- paste(sapply(seq_len(nrow(shots)), function(i) {
      s <- shots[i,]
      x <- s$LOC_X; y <- s$LOC_Y
      ...
      if(made) {
        sprintf("<circle cx='%s' cy='%s' r='5' fill='%s' stroke='%s' stroke-width='1' opacity='0.85'/>", round(x,1), round(y,1), col, col)
      } else {
        sprintf("<circle cx='%s' cy='%s' r='5' fill='none' stroke='%s' stroke-width='2' opacity='0.7'/>", round(x,1), round(y,1), col)
      }
    }), collapse="")
    ...
    full_svg <- sprintf(
      "<svg class='sc-court-svg' viewBox='-260 -60 520 490' xmlns='http://www.w3.org/2000/svg'>%s%s%s</svg>",
      court_svg, dots_svg, logo_svg)
    
    # Key
    away_logo <- paste0("https://a.espncdn.com/i/teamlogos/nba/500/scoreboard/", tolower(away_abv), ".png")
    home_logo <- paste0("https://a.espncdn.com/i/teamlogos/nba/500/scoreboard/", tolower(home_abv), ".png")
    
    tags$div(class="sc-court-wrap",
             HTML(full_svg),
             tags$div(class="sc-key",
                      tags$div(class="sc-key-item",
                               tags$img(src=away_logo, style="width:20px;height:20px;object-fit:contain;"),
                               tags$span(away_abv),
                      ...
                      tags$div(class="sc-key-item",
                               tags$img(src=home_logo, style="width:20px;height:20px;object-fit:contain;"),
                               tags$span(home_abv),
  ...
  output$sc_team_stats_ui <- renderUI({
    ...
    away_shots <- shots %>% filter(team_id == away_tid)
    home_shots <- shots %>% filter(team_id == home_tid)
    
    # FG%
    a_fgm <- sum(away_shots$scoring_play, na.rm=TRUE); a_fga <- nrow(away_shots)
    h_fgm <- sum(home_shots$scoring_play, na.rm=TRUE); h_fga <- nrow(home_shots)
    a_fg_pct <- if(a_fga>0) round(a_fgm/a_fga*100) else 0
    h_fg_pct <- if(h_fga>0) round(h_fgm/h_fga*100) else 0
    
    # 3P%
    a_3pa <- away_shots %>% filter(points_attempted == 3); a_3pm <- sum(a_3pa$scoring_play, na.rm=TRUE); a_3pa_n <- nrow(a_3pa)
    h_3pa <- home_shots %>% filter(points_attempted == 3); h_3pm <- sum(h_3pa$scoring_play, na.rm=TRUE); h_3pa_n <- nrow(h_3pa)
    a_3pct <- if(a_3pa_n>0) round(a_3pm/a_3pa_n*100) else 0
    h_3pct <- if(h_3pa_n>0) round(h_3pm/h_3pa_n*100) else 0
    
    # Turnovers
    a_tov <- nrow(df %>% filter(team_id == away_tid, grepl("turnover|Turnover", type_text, ignore.case=TRUE)))
    h_tov <- nrow(df %>% filter(team_id == home_tid, grepl("turnover|Turnover", type_text, ignore.case=TRUE)))
    
    # Rebounds
    a_reb <- nrow(df %>% filter(team_id == away_tid, grepl("rebound|Rebound", type_text, ignore.case=TRUE)))
    h_reb <- nrow(df %>% filter(team_id == home_tid, grepl("rebound|Rebound", type_text, ignore.case=TRUE)))
    
    # Final score
    last_play <- df %>% arrange(desc(game_play_number)) %>% slice(1)
    a_score <- last_play$away_score; h_score <- last_play$home_score
    
    away_color <- "#e53935"; home_color <- "#1565c0"
    
    make_stat_bar <- function(a_val, h_val, a_label, h_label, stat_name, a_detail="", h_detail="") {
      total <- a_val + h_val
      a_pct <- if(total>0) round(a_val/total*100) else 50
      h_pct <- 100 - a_pct
      sprintf(
        "<div class='sc-stat-row'><div class='sc-stat-away'>%s <span class='sc-stat-detail'>%s</span></div><div class='sc-stat-label'>%s</div><div class='sc-stat-home'><span class='sc-stat-detail'>%s</span> %s</div></div><div style='padding:0 8px;'><div class='sc-bar-wrap'><div class='sc-bar-away' style='width:%s%%;background:%s;'></div><div class='sc-bar-home' style='width:%s%%;background:%s;'></div></div></div>",
        a_label, a_detail, stat_name, h_detail, h_label, a_pct, away_color, h_pct, home_color)
    }
    ...
    tags$div(class="sc-stats-card",
             ...
             HTML(sprintf("<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;'><div style='display:flex;align-items:center;gap:6px;'><img src='%s' style='width:24px;height:24px;'/><span style='font-family:Rajdhani,sans-serif;font-weight:700;font-size:14px;color:var(--text-primary);'>%s</span></div><div style='display:flex;align-items:center;gap:6px;'><span style='font-family:Rajdhani,sans-serif;font-weight:700;font-size:14px;color:var(--text-primary);'>%s</span><img src='%s' style='width:24px;height:24px;'/></div></div>",
                          away_logo, away_abv, home_abv, home_logo)),
             HTML(make_stat_bar(a_fg_pct, h_fg_pct, paste0(a_fg_pct,"%"), paste0(h_fg_pct,"%"), "Field Goal %",
                                sprintf("(%s-%s)", a_fgm, a_fga), sprintf("(%s-%s)", h_fgm, h_fga))),
             HTML(make_stat_bar(a_3pct, h_3pct, paste0(a_3pct,"%"), paste0(h_3pct,"%"), "Three Point %",
                                sprintf("(%s-%s)", a_3pm, a_3pa_n), sprintf("(%s-%s)", h_3pm, h_3pa_n))),
             HTML(make_stat_bar(a_tov, h_tov, as.character(a_tov), as.character(h_tov), "Turnovers")),
             HTML(make_stat_bar(a_reb, h_reb, as.character(a_reb), as.character(h_reb), "Rebounds"))
    )
  })
  
  # ── Game Leaders (for shot chart sidebar) ──
  output$sc_game_leaders_ui <- renderUI({
    ...
    # Sum stats per player from PBP
    scoring <- df %>% filter(scoring_play == TRUE, PLAYER_NAME != "", !is.na(PLAYER_NAME)) %>%
      group_by(PLAYER_NAME, team_id) %>%
      summarise(pts = sum(score_value, na.rm=TRUE), .groups="drop")
    
    rebs <- df %>% filter(grepl("rebound|Rebound", type_text, ignore.case=TRUE), PLAYER_NAME != "") %>%
      group_by(PLAYER_NAME, team_id) %>% summarise(reb=n(), .groups="drop")
    
    assists <- df %>% filter(scoring_play == TRUE, !is.na(athlete_id_2), athlete_id_2 != "") %>%
      group_by(athlete_id_2) %>% summarise(ast=n(), .groups="drop")
    
    # Match athlete_id_2 back to player name
    id_to_name <- df %>% filter(PLAYER_NAME != "", !is.na(athlete_id_1)) %>%
      distinct(athlete_id_1, PLAYER_NAME, team_id)
    assists <- assists %>%
      left_join(id_to_name, by=c("athlete_id_2"="athlete_id_1")) %>%
      filter(!is.na(PLAYER_NAME))
    
    make_leader <- function(stat_df, stat_col, cat_name) {
      if(is.null(stat_df) || nrow(stat_df)==0) return("")
      top_away <- stat_df %>% filter(team_id == df$away_team_id[1]) %>% arrange(desc(.data[[stat_col]])) %>% slice(1)
      top_home <- stat_df %>% filter(team_id == df$home_team_id[1]) %>% arrange(desc(.data[[stat_col]])) %>% slice(1)
      
      side_html <- function(r, col) {
        if(nrow(r)==0) return("<span style='color:var(--text-muted);font-size:11px;'>—</span>")
        sprintf("<div style='display:flex;align-items:center;gap:8px;'><span style='font-family:Rajdhani,sans-serif;font-weight:700;font-size:20px;color:var(--text-primary);'>%s</span><span style='font-family:Calibri,sans-serif;font-size:12px;color:var(--text-secondary);'>%s</span></div>",
                r[[col]][1], r$PLAYER_NAME[1])
      }
      
      sprintf("<div style='margin-bottom:10px;'><div style='font-size:10px;font-family:Share Tech Mono,monospace;color:var(--text-muted);letter-spacing:.08em;margin-bottom:4px;'>%s</div><div style='display:flex;justify-content:space-between;'>%s%s</div></div>",
              cat_name, side_html(top_away, stat_col), side_html(top_home, stat_col))
    }
    
    tags$div(class="sc-stats-card",
             tags$div(class="sc-stats-title", "Game Leaders"),
             HTML(make_leader(scoring, "pts", "POINTS")),
             HTML(make_leader(rebs, "reb", "REBOUNDS")),
             HTML(make_leader(assists, "ast", "ASSISTS"))
    )
  })
  ...
  output$sc_oncourt_ui <- renderUI({
    ...
    if(pf == "ALL" && tf == "ALL") return(NULL)  # Only show when filtering
    
    if(pf != "ALL") shots <- shots %>% filter(PLAYER_NAME == pf)
    if(tf != "ALL") shots <- shots %>% filter(shot_category == tf)
    if(nrow(shots) == 0) return(NULL)
    
    # Get unique lineup combos
    lineups <- shots %>% select(home_P1:home_P5, away_P1:away_P5) %>% distinct()
    if(nrow(lineups) == 0) return(NULL)
    
    rows_html <- paste(sapply(seq_len(min(nrow(lineups), 50)), function(i) {
      r <- lineups[i,]
      home_ps <- paste(r$home_P1, r$home_P2, r$home_P3, r$home_P4, r$home_P5, sep=", ")
      away_ps <- paste(r$away_P1, r$away_P2, r$away_P3, r$away_P4, r$away_P5, sep=", ")
      sprintf("<tr><td>%s</td><td>%s</td></tr>", away_ps, home_ps)
    }), collapse="")
    
    tags$div(class="sc-oncourt",
             tags$div(class="sc-oncourt-title",
                      sprintf("Players On Court (%s unique lineups)", nrow(lineups))),
             HTML(sprintf("<table class='sc-lineup-table'><thead><tr><th>%s (Away)</th><th>%s (Home)</th></tr></thead><tbody>%s</tbody></table>",
                          df$away_team_abbrev[1], df$home_team_abbrev[1], rows_html))
    )
  })
  
  
  # ════════════════════════════════════════════════════════════════════════════
  # GAME CALENDAR — Team-filtered calendar view
  # ════════════════════════════════════════════════════════════════════════════
  
  output$gcal_ui_player <- renderUI({ uiOutput("gcal_shared_ui") })
  output$gcal_ui_team   <- renderUI({ uiOutput("gcal_shared_ui") })
  
  output$gcal_shared_ui <- renderUI({
    team <- gcal_team()
    
    tagList(
      # Team selector row (icon grid)
      tags$div(class="gcal-team-row",
               lapply(seq_len(nrow(NBA_TEAMS)), function(i) {
                 t <- NBA_TEAMS[i,]
                 cls <- paste("gcal-team-btn", if(!is.null(team) && team == t$abv) "active" else "")
                 tags$button(class=cls,
                             onclick=sprintf("Shiny.setInputValue('gcal_team_click','%s',{priority:'event'})", t$abv),
                             tags$img(src=t$logo))
               })
      ),
      # Calendar
  ...
  output$gcal_calendar_ui <- renderUI({
    team <- gcal_team()
    y <- gcal_cal_year(); m <- gcal_cal_month()
    first_day <- as.Date(paste(y, m, "01", sep="-"))
    last_day  <- as.Date(paste(ifelse(m==12,y+1,y), ifelse(m==12,1,m+1), "01", sep="-")) - 1
    start_dow <- as.integer(format(first_day, "%w"))
    cal_start <- first_day - start_dow
    ...
    team_sched <- team_sched %>% filter(game_date >= cal_start, game_date <= cal_start + 41)
    
    # Deduplicate
    team_sched <- team_sched %>% group_by(game_id) %>% slice(1) %>% ungroup()
    ...
    if(is.null(team)) {
      return(tagList(nav_html, tags$div(class="pr-placeholder", tags$p("Select a team to view their schedule"))))
    }
    ...
    day_cells <- lapply(0:41, function(i) {
      ...
      is_cur_mon <- format(cell_date, "%Y-%m") == format(first_day, "%Y-%m")
      is_today <- cell_date == today
      cell_class <- paste("cal-cell", if(!is_cur_mon) "other-month" else "", if(is_today) "today" else "")
      
      date_num <- tags$span(class="cal-date-num", cell_day)
      day_g <- team_sched[team_sched$game_date == cell_date, ]
      
      game_pills <- if(nrow(day_g) > 0) {
        lapply(seq_len(min(nrow(day_g), 3)), function(gi) {
          g <- day_g[gi,]
          tl <- if(!is.na(g$team_logo) && nchar(g$team_logo)>0) sprintf("<img src='%s'/>", g$team_logo) else ""
          ol <- if(!is.na(g$opp_logo) && nchar(g$opp_logo)>0) sprintf("<img src='%s'/>", g$opp_logo) else ""
          sym <- if(!is.na(g$home_away_sym)) trimws(g$home_away_sym) else "vs"
          gid <- as.character(g$game_id)
          HTML(sprintf("<div class='cal-game' onclick=\"Shiny.setInputValue('gcal_game_click','%s',{priority:'event'})\">%s<span class='cal-game-teams'>%s</span><span class='cal-game-sym'>%s</span>%s</div>",
                       gid, tl, g$team, sym, ol))
        })
      } else list()
  ...
  # ════════════════════════════════════════════════════════════════════════════
  # GAMECAST — Play-by-Play Feed
  # ════════════════════════════════════════════════════════════════════════════
  
  output$gc_ui <- renderUI({
    gid <- gc_game_id()
    if(is.null(gid) || gid == "") {
      return(tags$div(class="pr-placeholder", style="margin-top:60px;", tags$p("Select a game from the Game Calendar to view GameCast")))
    }
    ...
    tagList(
      tags$div(class="gc-layout",
               # Scoreboard
               tags$div(class="gc-scoreboard",
                        tags$div(class="gc-team-block away",
                                 tags$span(class="gc-team-name", away_abv),
                                 tags$img(class="gc-team-logo", src=away_logo)
                        ),
                        ...
                        tags$div(class="gc-team-block home",
                                 tags$img(class="gc-team-logo", src=home_logo),
                                 tags$span(class="gc-team-name", home_abv)
                        )
               ),
  ...
  output$gc_feed_ui <- renderUI({
    ...
    qf <- gc_qtr_filter()
    if(qf != "ALL") df <- df %>% filter(qtr == as.integer(qf))
    
    if(nrow(df) == 0) return(tags$div(style="padding:20px;color:var(--text-muted);","No plays for this filter"))
    ...
    # Build player name lookup from athlete IDs
    id_to_name <- df %>% filter(PLAYER_NAME != "", !is.na(athlete_id_1)) %>%
      distinct(athlete_id_1, PLAYER_NAME)
    id_map <- setNames(id_to_name$PLAYER_NAME, as.character(id_to_name$athlete_id_1))
    
    # Render plays (most recent first)
    df_rev <- df %>% arrange(desc(game_play_number))
    # Cap at 300 plays for performance
    df_show <- head(df_rev, 300)
    
    cards_html <- paste(sapply(seq_len(nrow(df_show)), function(i) {
      r <- df_show[i,]
      is_scoring <- isTRUE(r$scoring_play)
      is_home_team <- isTRUE(r$team_id == home_tid)
      team_logo <- if(is_home_team) home_logo else away_logo
      ...
      # Assist line
      assist_html <- ""
      if(is_scoring && !is.na(r$athlete_id_2) && as.character(r$athlete_id_2) %in% names(id_map)) {
        assist_name <- id_map[as.character(r$athlete_id_2)]
        assist_html <- sprintf("<div class='gc-play-assist'>AST: %s</div>", assist_name)
      }
      
      # Score display
      score_html <- ""
      if(is_scoring) {
        score_html <- sprintf("<div class='gc-play-score'><span class='away-s'>%s</span><span style='color:var(--text-muted);font-size:12px;margin:0 2px;'>-</span><span class='home-s'>%s</span></div>",
                              r$away_score, r$home_score)
      }
      
      card_cls <- paste("gc-play-card", if(is_scoring) "scoring" else "")
      
      sprintf("<div class='%s'><img class='gc-play-logo' src='%s'/><div class='gc-play-body'><div class='gc-play-header'><span class='gc-play-type'>%s</span><span class='gc-play-clock'>%s</span></div><div class='gc-play-text'>%s</div>%s</div>%s</div>",
              card_cls, team_logo, play_type, time_str, play_text, assist_html, score_html)
    }), collapse="")
